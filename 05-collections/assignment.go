...
func main() {
	...
	var countwords map[string]int = map[string]int{}
	var maxcount, maxword = 0, ""

	for _, word := range words {
		//fmt.Println(idx, word)
		if wcount, ok := countwords[word]; ok {
			countwords[word] = wcount + 1
		} else {
			countwords[word] = 1
		}
		if maxcount < countwords[word] {
...
//Fix the code ..not printing the KEy
